...
function applyThemeColorMetaSet() {
  // Couleurs fixes demandées : #ffb0ff (clair), #232323 (sombre)
  document.querySelectorAll('meta[name="theme-color"]').forEach((node) => node.remove());

  const variants = [
    { media: "(prefers-color-scheme: light)", content: "#ffb0ff" },
    { media: "(prefers-color-scheme: dark)", content: "#232323" }
  ];
...
function applyNavbarPaletteForPage(themeQuery, useBluePalette) {
  ...
  const navColor = isDark ? "#374785" : "#7494ec";
